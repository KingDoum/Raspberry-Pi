...
```
sudo apt update
sudo apt install locales libglib2.0-0 libxext6 libsm6 libxrender1
```

2. 下载Miniconda安装包:

根据树莓派的CPU架构下载对应版本,例如arm32版:

``` 
wget https://repo.anaconda.com/miniconda/Miniconda3-latest-Linux-aarch32.sh
```

3. 运行安装脚本:

```
bash Miniconda3-latest-Linux-aarch32.sh 
```

按照提示进行配置并安装到默认路径/opt/miniconda3
...
但一般不建议使用chmod 777,因为它给了所有人最高权限,会带来安全隐患。正确的做法是根据需要给不同用户设置合理的最小权限。
